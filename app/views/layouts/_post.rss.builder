xm.item do
  ...
  xm.guid [request.host_with_port+request.relative_url_root, post.category_id.to_s, post.topic_id.to_s, post.id.to_s].join(":"), "isPermaLink" => "false"
  xm.author "#{post.user.login}"
  xm.link topic_url(post.category_id, post.topic_id)
end
