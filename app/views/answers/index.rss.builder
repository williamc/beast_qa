...
xml.rss "version" => "2.0",
  ...
  xml.channel do
    xml.title "{search_answers_title} | Beast"[:answers_feed_title, search_answers_title]
    xml.link "http://#{request.host_with_port}#{search_answers_path}"
    ...
    unless params[:q].blank?
    	xml.tag! "opensearch:totalResults", @answers.total_entries
    	xml.tag! "opensearch:startIndex", (((params[:page] || 1).to_i - 1) * @answers.per_page)
